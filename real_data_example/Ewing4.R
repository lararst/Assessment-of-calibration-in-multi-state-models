...
progression0$Progrstat=1
Mod1 <- progression0[,c(1,3,15)]
#######################################################################
#wide format
#######################################################################
...
for  (i in 1:nrow(wideMod1) ){
  wideMod1$Progrstat[i]=0
  wideMod1$DEATHstat[i]=0
  
  wideMod1$Progrtime[i]=NA
  ...
  for (j in 1:nrow(Mod1) ){
    if(wideMod1$Patient_ID[i]==Mod1$Patient_ID[j]){
      if(Mod1$Progrstat[j]==1){
        wideMod1$Progrstat[i]=1
        wideMod1$Progrtime[i]=Mod1$Time_to_prog_confirmed[j]
        if(!is.na(wideMod1$Progrtime[i]) & !is.na(wideMod1$survtime[i]) & (wideMod1$Progrtime[i]==wideMod1$survtime[i])){
          wideMod1$Progrtime[i] = wideMod1$survtime[i] - 1
        }
      }
    }
  }
...
for  (i in 1:nrow(wideMod1) ){
  if(!is.na(wideMod1$survtime[i])){
    if(is.na(wideMod1$Progrtime[i])){
      wideMod1$Progrtime[i]=wideMod1$survtime[i]
    }
  }else{
    if(is.na(wideMod1$lastfollowup[i]) & is.na(wideMod1$Progrtime[i])){
      a <- c(a,i)
    }else{
      tmax=max(wideMod1$lastfollowup[i], wideMod1$Progrtime[i],  na.rm=T)
      if(is.na(wideMod1$Progrtime[i])){
        wideMod1$Progrtime[i]=tmax
      }
      if(is.na(wideMod1$survtime[i])){
...
#for  (i in 1:nrow(wideMod1) ){
#  if(is.na(wideMod1$survtime[i])){
#    if(is.na(wideMod1$lastfollowup[i]) & is.na(wideMod1$LR_DMOtime[i]) & 
#       is.na(wideMod1$DMPtime[i])){
#      a <- c(a,i)
#    }else{
#      tmax=max(wideMod1$lastfollowup[i], wideMod1$LR_DMOtime[i], wideMod1$DMPtime[i],  na.rm=T)
#      wideMod1$survtime[i]=tmax
#    }
#  }
#}
...
colnames(widedata)[10]<-"ChemoArm"
colnames(widedata)[9]<-"TumourBodyPart"
colnames(widedata)[8]<-"Pat_Age"
colnames(widedata)[7]<-"TumourVolume"
colnames(widedata)[6]<-"PAT_Sex"

#time calculated in years
widedata$Progrtime<-widedata$Progrtime/365
...
#########################################################################
#transition matrix
tmat <- transMat(x = list(c(2), 
                          c(3),
                          c()),
                 names = c("Rand", "Progr", "Death"))
tmat

#long format
covs <- c("PAT_Sex", "TumourVolume","Pat_Age", "TumourBodyPart","ChemoArm")
longdata <- msprep(time = c(NA, "Progrtime","survtime"), 
                   status = c(NA, "Progrstat", "DEATHstat"), 
